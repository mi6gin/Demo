...
def list_files(message):
    ...
    if message.text in dict:
        ...
        if message.text in ('1' , '2'): bot.register_next_step_handler(message, list_files)
    elif message.text in ('/wiki' , '/help' , '/start', '/list'):bot.send_message(message.chat.id, 'Простите мистер Романов, я совсем забыла вам сказать, что в режиме "списка" команды плохо работают... Отправте ещё раз пожалуйста.')
...
def handle_text(message):
    if message.text in ('/wiki' , '/help' , '/start', '/list'):bot.send_message(message.chat.id, 'Простите мистер Романов, я совсем забыла вам сказать, что в режиме "википедии" команды плохо работают... Отправте ещё раз пожалуйста.')
    else:
        machina=message.text.strip().lower()
        try:
            final_message=wikipedia.summary(machina)
        except wikipedia.exceptions.PageError:
            final_message="Нормально формулируйте мыcли мистер Романов!!! Я вас не понимаю!!! Я отказываюсь так работать!!!"
